    ...
    <>
      <header
        className={cn(
          ...
          isScrolled
            ? "py-1.5 sm:py-2 md:py-4 bg-white/80 backdrop-blur-2xl shadow-[0_10px_40px_rgba(0,0,0,0.05)]"
            : "py-1.5 sm:py-3 md:py-6 bg-white border-b border-slate-50"
        )}
      >
        <div className="container mx-auto px-3 md:px-6">
          <div className="flex items-center justify-between gap-2 md:gap-8">
            ...
            <div className="flex items-center gap-2 md:gap-6 min-w-0">
              <Link href={`/${slug}`} className="flex items-center gap-2 md:gap-4 group min-w-0">
                <div className="w-9 h-9 md:w-16 md:h-16 bg-slate-950 rounded-lg md:rounded-3xl flex items-center justify-center shadow-2xl shadow-slate-950/20 group-hover:rotate-6 transition-all duration-500 shrink-0">
                  {settings?.logo ? (
                    <img src={settings.logo} alt="Logo" className="w-full h-full object-cover rounded-lg md:rounded-3xl" />
                  ) : (
                    <Utensils className="text-primary group-hover:scale-110 transition-transform duration-500" size={15} />
                  )}
                </div>
                <div className="flex flex-col min-w-0">
                  <h1 className="text-[10px] sm:text-heading-2 md:text-heading-1 font-display font-bold text-slate-950 tracking-tighter leading-none uppercase truncate max-w-32 sm:max-w-none">
                    {settings?.storeName?.split(' ')[0] || "FOOD"}<span className="hidden sm:inline text-primary">{settings?.storeName?.split(' ')[1] || "SYSTEM"}</span>
                  </h1>
                  <p className="hidden xs:flex text-[7px] md:text-label font-body font-medium text-slate-400 uppercase tracking-[0.06em] mt-0.5 md:mt-1 pr-2 items-center gap-1 whitespace-nowrap">
            ...
            <div
              ...
              className="hidden lg:flex items-center gap-3 md:gap-5 bg-white shadow-xl shadow-slate-100/50 px-4 md:px-8 py-2 md:py-4 rounded-2xl md:rounded-3xl border border-slate-50 cursor-pointer hover:border-primary/20 hover:shadow-primary/5 transition-all duration-500 group"
            >
            ...
            <div className="flex items-center gap-1 md:gap-6 shrink-0">
              ...
              <div className="hidden md:flex relative items-center group">
                ...
                <input
                  ...
                  className="h-12 md:h-16 w-40 md:w-56 lg:w-80 pl-12 md:pl-16 pr-4 md:pr-6 bg-slate-50 border border-transparent rounded-2xl md:rounded-3xl focus:bg-white focus:ring-[6px] focus:ring-primary/5 focus:border-primary/20 transition-all duration-300 text-[10px] md:text-label font-body font-medium uppercase tracking-[0.06em] outline-none"
                />
              </div>
              ...
              <button
                onClick={onOpenMenu}
                className="w-9 h-9 md:w-16 md:h-16 bg-white border border-slate-100 rounded-lg md:rounded-2xl flex items-center justify-center text-slate-950 shadow-sm active:scale-90 transition-all"
              >
