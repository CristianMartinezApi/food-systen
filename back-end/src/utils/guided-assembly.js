function validateGuidedSelections(groups, selections) {
  ...
  for (const selection of selections) {
    ...
    if (selectedCount < min) {
      throw new Error(
        `Grupo "${group.name}" deve ter pelo menos ${min} seleção(ões).`,
      );
    }

    if (selectedCount > max) {
      throw new Error(
        `Grupo "${group.name}" permite no máximo ${max} seleção(ões).`,
      );
    }
  }
...
module.exports = { validateGuidedSelections };
